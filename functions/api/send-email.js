...
async function handleRequest(request, env) {
  ...
  if (!resendApiKey) {
    ...
    return jsonResponse(503, { ok: false, message: 'Hệ thống email chưa được cấu hình. Vui lòng liên hệ hotline.' });
  }
  ...
  const recipients = (env?.LEAD_NOTIFICATION_EMAIL || runtimeEnv.LEAD_NOTIFICATION_EMAIL || RECIPIENT)
    ...
    .filter(Boolean);
  const from = env?.RESEND_FROM_EMAIL || runtimeEnv.RESEND_FROM_EMAIL || 'TUDO EDU <onboarding@resend.dev>';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return jsonResponse(502, { ok: false, message: 'Email chưa gửi được. Vui lòng gọi hotline để được hỗ trợ.' });
    }
